...
export default function EventDetailPage() {
    ...
    if (typeof window !== "undefined") {
      ...
      if (stored) {
        try {
          ...
          console.error("Failed to parse user storage in event details:", e);
        }
      }
    }
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen bg-background">
        {user && (
          <DashboardSidebar
            role={user?.roles?.[0] || user?.role || "alumni"}
  ...
  if (error || !event) {
    return (
      <div className="flex min-h-screen bg-background">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <Navbar />
          <div className="w-full px-6 py-24 text-center space-y-4 flex flex-col items-center justify-center flex-1">
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {user && (
        <DashboardSidebar
          role={(user?.role || user?.roles?.[0] || "alumni") as any}
      ...
      <SidebarInset>
        ...
        <div className="w-full px-6 py-6 md:px-8 space-y-6 flex-1">
          <Link
            href="/events"
