...
import { useMyReferralsQuery } from "@/hooks/queries/referrals";
...
export default function MyReferralsPage() {
  const { user } = useAuth();
  const { data: referrals = [], isLoading, error: queryError, refetch, isRefetching } = useMyReferralsQuery();
  const [activeFilter, setActiveFilter] = useState<"ALL" | ReferralRequest["status"]>("ALL");

  const error = queryError ? (queryError instanceof Error ? queryError.message : "Failed to load referrals.") : null;
    ...
    <div className="w-full px-4 sm:px-6 pb-6 space-y-6">
      ...
      {!isLoading && error && (
        <Alert variant="destructive">
          ...
          <Button onClick={() => refetch()} variant="outline" size="sm" className="mt-3 cursor-pointer">
            Retry
          </Button>
        </Alert>
      )}
